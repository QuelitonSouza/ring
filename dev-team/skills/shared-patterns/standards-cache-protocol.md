...
Orchestrators (the running-dev-cycle and reviewing-code skills) MUST use this exact
block shape when injecting cached standards into a reviewer's dispatch prompt:

```
<standards>
  <standard url="https://raw.githubusercontent.com/QuelitonSouza/ring/main/.../file.md">
...
## For Orchestrators (dev-cycle)

At Step 1.5 of the cycle:

1. Detect project stack (C#/.NET / TypeScript / Frontend).
...
At dispatch time (Step 3 of the reviewing-code skill, and elsewhere): build the
`<standards>` block per the canonical format above, one `<standard>` entry per URL

...
- Backend: Gate 8 (Review — default reviewers plus triggered specialists), Gate 9 (Validation — aggregates every task's acceptance criteria + one human approval, after Gate 8)
...
| "This epic has only one task, so cadence doesn't matter" | Cadence is a schema-level invariant enforced by the gate-progression validation and the state-write paths documented in the running-dev-cycle skill. Bypassing it writes state to the wrong path and breaks the progression check for the next epic that has multiple tasks. | **MUST follow the documented cadence regardless of task count. Treat single-task epics as "tasks: [epic-itself]" for state purposes.** |
